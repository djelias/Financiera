...
@section('container')
 <br>
      <div class="row">
        ...
        @endcan
         {{ Form::open(['route'=>'prestatario.store', 'method'=>'POST', 'class'=>'agregar']) }}
             @include('prestatario.form_master')
             {{ form::close() }}
      </div>
         <div class="col-md-4">
        {!! Form::open(['route'=>'prestatario.index', 'method'=>'GET', 'class'=>'navbar-form pull-right', 'role'=>'search'])!!}
        <div class="input-group"> 
            {!! Form::text('pnombre', null, ['class'=>'form-control', 'placeholder'=>'Buscar'])!!}
  ...
  <table class="table table-striped" style="text-align:center" >
    ...
    <?php $no=1; ?>
    @foreach ($prestatarios as $key => $value)
    <tr>
        <td>{{$no++}}</td>
        <td>{{ $value->pnombre }} {{ $value->snombre }} {{ $value->papellido }}</td>
        <td>
